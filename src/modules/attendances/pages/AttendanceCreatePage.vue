          ...
          <v-row>
            ...
            <v-col cols="12" md="6">
              <v-select
                ...
                hint="Selecione o agente que realizou o atendimento"
                persistent-hint
              ></v-select>
            </v-col>
            ...
            <v-col cols="12" md="6">
              <v-select
                ...
                hint="Como o atendimento foi realizado"
                persistent-hint
              >
                <template #item="{ item, props: itemProps }">
                  <v-list-item v-bind="itemProps">
                    <template #prepend>
                      <v-icon :color="getChannelColor(item.value)" class="mr-3">
                        {{ getChannelIcon(item.value) }}
                      </v-icon>
                    </template>
                    <v-list-item-title>{{ item.title }}</v-list-item-title>
                  </v-list-item>
            ...
            <v-col cols="12" md="6">
              <v-text-field
                ...
                hint="Quando o atendimento começou"
                persistent-hint
              ></v-text-field>
            </v-col>
            ...
            <v-col cols="12" md="6">
              <v-text-field
                ...
                label="Data/Hora de Término (Opcional)"
                type="datetime-local"
                variant="outlined"
            ...
            <v-col cols="12" md="6">
              <v-text-field
                v-model="formData.scheduled_visit_at"
                label="Agendar Visita (Opcional)"
                type="datetime-local"
                variant="outlined"
                prepend-inner-icon="mdi-calendar-plus"
                hint="Data/hora para agendar uma visita relacionada"
                persistent-hint
              ></v-text-field>
            </v-col>
...
const loadProperties = async () => {
  ...
  try {
    properties.value = await propertiesService.listProperties({ limit: 1000 })
  } catch (error) {
